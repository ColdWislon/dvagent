...
5. First person on a new setup (or after wrapper changes): run
   ...
   for everyone.
6. Brand new to the repo (or inheriting a block)? Type `/start-here` in
   Copilot chat: a read-only guided session that tours the environment,
   runs the smoke test with you, and hands you ranked first tasks. On
   repos generated by `uvm-gen/`, read the generated `GETTING_STARTED.md`
   first — same path, no Copilot required.
