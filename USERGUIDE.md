...
(RTL read-only, no checker weakening, no exclusions) are enforced by the
CI gate on every MR — agent-authored or not.
...
5. First person on a new setup (or after wrapper changes): run
   `/learn-dv-wrapper` and answer its questions. That knowledge persists
   for everyone.
...
## A typical session (closing VP-AXI-041)

1. Fresh worktree, fresh chat, pick dv-test-writer:
   `/close-vplan-item VP-AXI-041 axi_bridge`
...
- **No claim without a verdict.** If an agent tells you something passes
  without showing the JSON verdict, that's a defect — call it out (and
...
- **Thrashing** (repeated similar failed attempts): stop the session.
  ...
  feedback we want.
- **Unparseable or surprising `dv` output**: the agent should stop and
  ask. If wrapper behavior changed, re-run `/learn-dv-wrapper`.
...
- **"It worked yesterday" / sudden failures without TB changes**: the
  ...
  (RTL git log since last pass, optional bisect). Never let an agent
  "fix" the TB to match changed RTL ports without the change note; if
  it asks you for one, that's the process working.
...
you answer wrapper questions honestly, and you attach session reports to
MRs. Periodic human audits of merged MRs are part of the methodology —
clean reports are trusted because we verify they deserve it.
